...
| `cilium-k8s` | `pkg/k8s/utils/utils_test.go`, `pkg/k8s/client/testutils/script_test.go`, `pkg/k8s/tables/script_test.go` | Pure K8s resource metadata, slim pod/node types, label selectors, and an in-process watch store are present. | `cargo test -p cilium-k8s`; workspace build. | `2/5` |
